...
class Model(pl.LightningModule):
	...
	def perform_loss_step(self, batch, mode='train'):
		...
		assert self.masking == "single" or self.masking == "multi"

		if self.masking == "single":

			''' Masking single bin through whole training'''
			...
			if self.attn_mask is None:
				...
				attn_mask = torch.cat([attn_mask_base[i].repeat(self.num_heads, self.seq_size, 1) 
						   		for i in range(attn_mask_base.shape[0])]).to(self.device) # batch_size x seq_size x seq_size
			else:
			...
			preds = torch.squeeze(all_logits[torch.arange(bins.shape[0]), missing_ids])

			# preds = torch.argmax(pred_logits, dim=-1)	
			correct = torch.sum(torch.round(preds).long() == labels)

			'''NOISED LABELS OPTION'''
			# noised_labels = labels.float() - torch.rand(labels.shape).to(self.device) + .5
			# loss = nn.MSELoss()(preds, noised_labels)
		...
		elif self.masking == "multi":

			'''	Increase amount of bins masked during training process'''
